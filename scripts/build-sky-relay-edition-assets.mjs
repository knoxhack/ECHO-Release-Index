...
const EDITIONS = [
  {
    key: 'native',
    repoName: 'ECHO-Sky-Relay-Native-Edition',
    template: 'release-manifest.template.json',
    family: 'echo-addon',
    artifactFolder: 'addons',
    tag: 'sky-relay-native-0.1.0-alpha',
  },
  {
    key: 'neoforge',
    repoName: 'ECHO-Sky-Relay-NeoForge-Edition',
    template: 'release-manifest.template.json',
    family: 'neoforge',
    artifactFolder: 'mods',
    tag: 'sky-relay-neoforge-0.1.0-alpha',
  },
  {
    key: 'standalone',
    repoName: 'ECHO-Sky-Relay-Standalone-Edition',
    template: 'release-manifest.template.json',
    family: 'standalone',
    artifactFolder: 'mods',
    tag: 'sky-relay-standalone-0.1.0-alpha',
  },
]
...
function usage() {
  return `Usage: node scripts/build-sky-relay-edition-assets.mjs [options]

Builds Sky Relay edition pack assets from a verified ECHO-Modules release stage.
...
Options:
  ...
  --module-release-dir <path>  Module release stage with echo-release.json.
                               Default: ../ECHO-Modules/tmp/sky-relay-runtime-module-release
  --out-root <path>            Output root. Default: tmp/sky-relay-edition-assets
...
function parseArgs(argv) {
  ...
  const args = {
    ...
    outRoot: path.resolve(root, 'tmp', 'sky-relay-edition-assets'),
  ...
  args.moduleReleaseDir = path.resolve(args.workspaceRoot, 'ECHO-Modules', 'tmp', 'sky-relay-runtime-module-release')
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--workspace-root') {
      ...
      if (!args.moduleReleaseDir) {
        args.moduleReleaseDir = path.resolve(args.workspaceRoot, 'ECHO-Modules', 'tmp', 'sky-relay-runtime-module-release')
      }
    } else if (arg === '--module-release-dir') {
      args.moduleReleaseDir = path.resolve(next())
      ...
      args.only = new Set(next().split(',').map((item) => item.trim().toLowerCase()).filter(Boolean))
    } else if (arg === '--clean') {
      args.clean = true
    } else if (arg === '--help') {
      args.help = true
...
function nativeLoaderManifest(minecraftVersion) {
  ...
  const mainClass = process.env.ECHO_NATIVE_LOADER_MAIN_CLASS || 'com.echo.NativeLoaderClient'
  return {
    version,
...
function neoforgeLoaderManifest(minecraftVersion) {
  ...
  return {
    ...
    versionJson: {
      ...
      libraries: [
        {
          name: `net.neoforged:neoforge:${version}`,
        },
      ],
    },
    libraries: [
...
async function resolveModuleRequirements(moduleReleaseDir, moduleRelease, edition) {
  ...
  const requirements = []
  for (const moduleId of RELEASE_MODULE_IDS) {
    ...
    if (record.sha256 !== String(artifact.sha256).toLowerCase()) {
      throw new Error(`${artifact.filename} SHA-256 differs from echo-release.json.`)
    }
    if (record.size !== Number(artifact.size)) {
      throw new Error(`${artifact.filename} size differs from echo-release.json.`)
    }
    const installPath = `${edition.artifactFolder}/${artifact.filename}`
    requirements.push({
      id: moduleId,
      moduleId,
...
function packManifestBase({ template, edition, requirements, version, channel, zipName }) {
  const minecraftVersion = process.env.ECHO_SKY_RELAY_MINECRAFT_VERSION || DEFAULT_MINECRAFT_VERSION
  ...
  const manifest = {
    ...
    launch: {
      mainClass: edition.key === 'standalone'
        ? 'com.echo.skyrelay.standalone.SkyRelayStandaloneMain'
        : edition.key === 'native'
    ...
    changelog: [
      `${template.displayName} alpha pack assembled from compiled ECHO modules.`,
      'Includes Sky Relay protocol content plus Terminal, Lens, HoloMap, PowerGrid, WeatherCore, Recovery, LogisticsNetwork, NetCore, AdapterCore, RuntimeGuard, and Core modules.',
      'This draft remains gated on launcher install/update/repair/rollback and gameplay smoke evidence.',
    ],
...
async function buildEdition({ args, edition, moduleRelease }) {
  ...
  const templatePath = path.join(repoRoot, edition.template)
  const template = await readJson(templatePath)
  if (template.packId !== `sky-relay-${edition.key}-edition`) {
  ...
  const baseManifest = packManifestBase({
    template,
    edition,
    requirements,
  ...
  const exportReport = {
    ok: true,
    schemaVersion: 'echo.skyrelay.pack-export-report.v1',
    ...
    artifactFamily: edition.family,
    plannedButNotPackaged: PLANNED_NOT_BUILT_MODULE_IDS,
  }
  ...
  const releaseManifest = {
    ...
    assets: [
      {
        ...
        role: 'pack-manifest',
        sha256: packManifestRecord.sha256,
        size: packManifestRecord.size,
      },
      {
        name: zipName,
        role: 'pack-artifact',
    ...
    notes: [
      ...
      'This release remains draft-only until launcher and gameplay smoke evidence passes.',
    ],
  }
  await writeJson(path.join(stage, 'echo-release.json'), releaseManifest)
  await writeJson(path.join(stage, 'sky-relay-pack-build-report.json'), {
    schemaVersion: 'echo.skyrelay.pack-build-report.v1',
    generatedAt: new Date().toISOString(),
    repoName: edition.repoName,
    packId: template.packId,
    releaseTag: edition.tag,
    stage,
    ...
    plannedButNotPackaged: PLANNED_NOT_BUILT_MODULE_IDS,
    promotionBlockers: [
      'Launcher install/update/repair/rollback smoke must run against downloaded draft assets before catalog approval.',
      'Gameplay smoke evidence for first 30 minutes, first 2 hours, and Signal Crown completion is still required.',
    ],
  })
  ...
  return {
    ...
    assets: [
      ...
      await fileRecord(path.join(stage, 'echo-release.json')),
      packManifestRecord,
      zipRecord,
      await fileRecord(path.join(stage, 'sky-relay-pack-build-report.json')),
...
async function main() {
  ...
  const selected = EDITIONS.filter((edition) => {
    if (!args.only) return true
    return args.only.has(edition.key) || args.only.has(edition.repoName.toLowerCase())
  })
  const editions = []
  for (const edition of selected) {
    editions.push(await buildEdition({ args, edition, moduleRelease }))
  }

  const report = {
    ok: true,
    schemaVersion: 'echo.skyrelay.edition-assets-build.v1',
    generatedAt: new Date().toISOString(),
  ...
  await writeJson(path.join(args.outRoot, 'sky-relay-edition-assets-build-report.json'), report)
