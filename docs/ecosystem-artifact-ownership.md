...
| `knoxhack/ECHO-Modules` | First-party module source plus `.echo-addon`, `-neoforge.jar`, `-standalone.jar`, `-sources.jar`, per-module `.echo/content-graph/` release assets, and release-root `content-graph-evidence.json`. | `modules/*.json` records imported module artifacts, dependency edges, trust, validation, pack compatibility, and the `content-graph-evidence` role when the canonical release evidence artifact is available. |
...
| `knoxhack/ECHO-Native-Platform` | Native runtime, loader, contracts, diagnostics, and PackOS integration assets. | `products/native-platform.json` records loader/runtime artifacts only; pack compatibility and modules stay in pack-owned catalog entries. |
